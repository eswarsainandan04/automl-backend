...
8. Run Optuna HPO for each candidate model (fast mode may skip baseline models)
...
def _resolve_hpo_budget(row_count: int, model_count: int) -> Tuple[int, int]:
    ...
    per_model = max(5, int(total_budget / max(1, model_count)))
    per_model = min(20, per_model)
...
def _is_baseline_model(model_name: str) -> bool:
    return model_name in _BASELINE_MODELS
...
def run_model_selection_supabase(
    ...
    for model_name, scaling_flag in available_candidates:
        if not fast_mode_active and total_hpo_time >= HPO_FAST_MODE_AFTER_SEC:
            fast_mode_active = True
            logger.warning(
                "HPO fast mode enabled after %.1fs; baseline models will skip HPO.",
                total_hpo_time,
            )

        skip_hpo = fast_mode_active and _is_baseline_model(model_name)
        trials_for_model = 0 if skip_hpo else per_model_trials
        skip_reason = "fast_mode" if skip_hpo else None
        if skip_hpo:
            logger.info("Fast mode: skipping HPO for baseline model '%s'.", model_name)
    ...
    hpo_summary = {
        ...
        "top_k": top_k,
        "fast_mode": {
            "enabled": fast_mode_active,
            "threshold_seconds": HPO_FAST_MODE_AFTER_SEC,
        },
    }
