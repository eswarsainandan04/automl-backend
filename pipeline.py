...
5. missing_values       → sklearn-based imputation (numeric, temporal,
                          categorical). Runs in a subprocess using the
                          backend Python (or MISSING_VALUES_PYTHON when set).
                          Executable: MISSING_VALUES_PYTHON (configured below)
                          Output: updates *_cleaned.csv in-place
...
# Python executable that runs the missing-values step.
# Step 5 runs in a subprocess and defaults to the current backend interpreter.
def _resolve_missing_values_python() -> str:
    """Return the best available Python executable for missing-values step."""
    env_python = os.getenv("MISSING_VALUES_PYTHON")
    if env_python:
        return env_python
    ...
    raise FileNotFoundError(
        ...
        "Set MISSING_VALUES_PYTHON explicitly."
    )


MISSING_VALUES_PYTHON = _resolve_missing_values_python()

# Absolute path to the standalone runner script for step 5.
# It imports SklearnMissingValueHandler and calls process_all_datasets().
...
def _run_step_missing_values(user_id: str, session_id: str, status: dict) -> bool:
    """
    Step 5 — Missing Values Handler (sklearn)
    Imputes missing values using sklearn-based strategies.

    WHY a subprocess?
    Running it as a subprocess keeps preprocessing isolated from the main
    FastAPI process.
    """
    _set_step(status, "missing_values", "running")
    try:
        result = subprocess.run(
            [MISSING_VALUES_PYTHON, MISSING_VALUES_RUNNER, user_id, session_id],
            capture_output=True,
            text=True,
...
def _run_step_structural_type(user_id: str, session_id: str, status: dict) -> bool:
    ...
    Also writes per-column summary metrics to *_summary.json.
    """
    ...
    try:
        ...
        from data_preprocessing.column_summary import process_user_datasets as build_column_summary
        ...
        build_column_summary(user_id, session_id)
