...
5. missing_values       → AutoGluon-based imputation (numeric, temporal,
                          categorical). Runs in a dedicated virtual-env
                          subprocess because AutoGluon is NOT installed in
                          the main backend environment.
...
# The main backend venv does NOT have AutoGluon, so step 5 is run in a
# dedicated venv as a subprocess.
def _resolve_autogluon_python() -> str:
    ...
    # Local Windows fallback for existing developer setup.
    return r"C:\FinalYearProject\Project\backend\pre-processing\Scripts\python.exe"
